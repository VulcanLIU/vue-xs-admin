...
export default {
	...
	setup() {
		...
		const chartOptions = {
			...
			layout: { padding: { top: 20, left: 10, right: 10 } }, // 图表内边距
			scales: {
				// 坐标轴配置
				y: {
					// 纵轴（数值轴）
					...
					beginAtZero: true,
					ticks: { stepSize: 1, max: 100 }, // 刻度间隔10，最大值100
				},
				y1: {
					type: "linear",
					display: true,
					...
					grid: {
						drawOnChartArea: false, // 只显示一侧的网格线
					},
					// 你可以根据实际需求设置ticks等
				},
				x: {
			...
			plugins: {
				...
				title: {
					...
					display: true, // 显示标题
					text: "部附件生产进度统计", // 标题内容
					font: { size: 25, weight: "bold" }, // 标题字体（16号加粗）
				},
				annotation: {
					// 标注插件（当前空配置）
					annotations: [],
		...
		const initChart = async (args = "") => {
			...
			console.log(`cylinderData=${optionDataMap[selectedOption.value]}`);
			...
			<ElRow>
				<ElCol :span="11">
					<div class="vertical-center">
						<span style="font-size: medium; color: black">修理方式</span>
					</div>
				</ElCol>
				<ElCol :span="11">
...
.chart-wrapper {
	...
	margin: 0px auto 0; /* 顶部边距80px，左右自动居中 */
}
