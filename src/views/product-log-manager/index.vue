<script>
  ...
  import { dropdownOptions, lineDataMap, optionDataMap, steps } from './component/form/fetchdata';
  ...
  export default {
    ...
    setup() {
      ...
      // 监听下拉选项变化（同时更新柱状图和折线图数据）
      watch(selectedOption, () => {
        if (window.myChart) {
          window.myChart.data.datasets[0].data = optionDataMap[selectedOption.value]; // 更新柱状图数据
          window.myChart.data.datasets[1].data = lineDataMap[selectedOption.value]; // 更新折线图数据
      ...
      watch(selectedOption, () => {
        if (window.myChart) {
          // 图表实例存在时
          // 更新柱状图数据集为当前选中选项的数据
          window.myChart.data.datasets[0].data = optionDataMap[selectedOption.value];
          // 触发图表更新（重新渲染）
          window.myChart.update();
        }
      });
...
<style scoped>
  ...
  .dropdown-container {
    ...
    background-color: rgb(21, 2, 2); /* 背景色 */
    padding: 8px; /* 内边距 */
