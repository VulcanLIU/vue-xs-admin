...
import TaskDiolog from "./TaskDialog.vue";
	...
	<div ref="sortable-ref" class="task-list">
		<ElCard class="list-item" draggable="true" shadow="hover">
			<div class="card-header handle" @click="centerDialogVisible = true">
				<article
					class="group flex flex-col bg-white dark:bg-dark-grey rounded-lg cursor-pointer shadow-task"
				>
					<h3
						class="text-base text-black dark:text-white font-bold select-none pointer-events-none"
					>
					...
					<p
						class="text-sm text-medium-grey font-bold select-none pointer-events-none whitespace-pre-wrap"
					>
						{{ `节点:\t${taskData.node} \n下发人:\t${taskData.issuer}` }}
					</p>
				</article>
