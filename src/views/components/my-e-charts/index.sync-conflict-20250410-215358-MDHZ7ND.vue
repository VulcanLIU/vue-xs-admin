<script lang="ts" setup>
  ...
  const dbInterfaceRef = ref<InstanceType<typeof DatabaseInterface>>();
  ...
  const handleFormSubmit = async (content: string) => {
    if (!dbInterfaceRef.value) return;

    try {
      await dbInterfaceRef.value.submitData(content);
    } catch (error) {
...
<template>
  <div class="page-container">
    ...
    <Form ref="formRef" :form-data="form" :form-option="formOption" @submit-form="handleFormSubmit" />
    <DatabaseInterface ref="dbInterfaceRef" />
  </div>
</template>
